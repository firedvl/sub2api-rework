...
func applyUpstreamModelMetadataToCodexDescriptor(
	...
	if value := bytes.TrimSpace(capabilities["apply_patch_tool_type"]); len(value) > 0 {
		_ = json.Unmarshal(value, &descriptor.ApplyPatchToolType)
	}
	if value := bytes.TrimSpace(capabilities["comp_hash"]); len(value) > 0 {
		_ = json.Unmarshal(value, &descriptor.CompHash)
	}
	if value := bytes.TrimSpace(capabilities["tool_mode"]); len(value) > 0 {
		_ = json.Unmarshal(value, &descriptor.ToolMode)
	}
	if value := bytes.TrimSpace(capabilities["use_responses_lite"]); len(value) > 0 {
		_ = json.Unmarshal(value, &descriptor.UseResponsesLite)
	}
	if value := bytes.TrimSpace(capabilities["multi_agent_reasoning_effort"]); len(value) > 0 {
		_ = json.Unmarshal(value, &descriptor.MultiAgentReasoningEffort)
	}
	if value := bytes.TrimSpace(capabilities["multi_agent_version"]); len(value) > 0 {
		_ = json.Unmarshal(value, &descriptor.MultiAgentVersion)
	}
}
