...
func TestSanitizeOpenAIResponsesInputItemIDsStripsOversizedKnownIDs(t *testing.T) {
	body := []byte(`{"input":[{"type":"web_search_call","id":"ws_` + strings.Repeat("x", 61) + `"},{"type":"web_search_call","id":"ws_` + strings.Repeat("x", 62) + `"}]}`)
	sanitized, changed, err := sanitizeOpenAIResponsesInputItemIDs(body)
	require.NoError(t, err)
	require.True(t, changed)
	require.True(t, gjson.GetBytes(sanitized, "input.0.id").Exists())
