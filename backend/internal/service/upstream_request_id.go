...
func UpstreamRequestIDHeaderName(account *Account) string {
	...
	name := strings.TrimSpace(account.GetExtraString(AccountExtraUpstreamRequestIDHeader))
	if isSensitiveUpstreamRequestIDHeader(name) {
		return ""
	}
	return name
}

func isSensitiveUpstreamRequestIDHeader(name string) bool {
...
func ValidateUpstreamRequestIDHeaderExtra(extra map[string]any) error {
	...
	if isSensitiveUpstreamRequestIDHeader(name) {
		return infraerrors.BadRequest("INVALID_UPSTREAM_REQUEST_ID_HEADER", "credential and cookie headers cannot be used as request identifiers")
	}
