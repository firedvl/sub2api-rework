...
func TestReasoningEffortBillingMultiplier(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.effort, func(t *testing.T) {
			require.Equal(t, tc.want, reasoningEffortBillingMultiplier("custom-model", tc.effort, multipliers))
		})
	}
	require.Equal(t, 1.0, reasoningEffortBillingMultiplier("custom-model", "max", nil))
	require.Equal(t, 3.0, reasoningEffortBillingMultiplier("claude-fable-5-1", "max", nil))
	require.Equal(t, 1.0, reasoningEffortBillingMultiplier("claude-fable-5-1", "high", nil))
	require.Equal(t, 1.0, reasoningEffortBillingMultiplier("claude-fable-5-1", "max", map[string]float64{"max": 1}))
	for _, invalid := range []float64{0, -1, math.NaN(), math.Inf(1)} {
		require.Equal(t, 1.0, reasoningEffortBillingMultiplier("custom-model", "max", map[string]float64{"max": invalid}))
	}
}

func TestOpenAIUsageWithoutResolverPreservesFableMaxDefault(t *testing.T) {
	...
	standard, err := svc.calculateOpenAIRecordUsageTokenCost(context.Background(), &APIKey{}, "claude-fable-5-1", 1, time.Time{}, tokens, "", "high", nil)
	require.NoError(t, err)
	maximum, err := svc.calculateOpenAIRecordUsageTokenCost(context.Background(), &APIKey{}, "claude-fable-5-1", 1, time.Time{}, tokens, "", "max", nil)
	require.NoError(t, err)
	require.InDelta(t, standard.TotalCost*3, maximum.TotalCost, 1e-12)
	require.InDelta(t, standard.ActualCost*3, maximum.ActualCost, 1e-12)
}
	...
	for _, effort := range []string{"none", "minimal"} {
		t.Run(effort, func(t *testing.T) {
			for _, body := range [][]byte{
				...
				require.Equal(t, 0.5, reasoningEffortBillingMultiplier("gpt-5.4", *got, map[string]float64{effort: 0.5}))
