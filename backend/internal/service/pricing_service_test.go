...
func TestFrontierModelPricingCatalogAndFallbackAgree(t *testing.T) {
	data, err := os.ReadFile(filepath.Join("..", "..", "resources", "model-pricing", "model_prices_and_context_window.json"))
	require.NoError(t, err)
	...
	for name, pricingService := range map[string]*PricingService{
		...
		t.Run(name, func(t *testing.T) {
			...
			for _, tc := range []struct {
				...
				{"claude-opus-5-5", 4e-6, 20e-6, 5e-6, 0.2e-6, 0},
			} {
				t.Run(tc.model, func(t *testing.T) {
					pricing, err := billing.GetModelPricing(tc.model)
					require.NoError(t, err)
					...
					require.Equal(t, tc.threshold, pricing.LongContextInputThreshold)
				})
			}
		})
	}
