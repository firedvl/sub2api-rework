...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	if useHTTPBridge {
		...
		for turn := 1; ; turn++ {
			...
			if invalidDigests := s.sessionInvalidEncryptedContentDigests(c, groupID, sessionHash); len(invalidDigests) > 0 {
			...
			if eventType == "error" {
				...
				if fallbackReason == openAIWSFallbackReasonInvalidEncryptedContent {
					...
					if digests := collectOpenAIEncryptedContentDigestsRaw(payload); len(digests) > 0 {
						s.markOpenAIWSInvalidEncryptedContentLineage(c, groupID, sessionHash, digests)
						logOpenAIWSModeInfo(
