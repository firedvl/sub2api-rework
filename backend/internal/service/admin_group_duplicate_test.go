...
func TestDuplicateGroupCopiesConfigurationDeeplyAndResetsRuntimeState(t *testing.T) {
	...
	source := &Group{
		...
		ModelsListConfig:            GroupModelsListConfig{Enabled: true, Models: []string{"display-only-alias"}},
		RPMLimit:                    99,
	...
	duplicate.ModelsListConfig.Models[0] = "changed-display"
	...
	*duplicate.DailyLimitUSD = 999
	...
	require.Equal(t, "display-only-alias", source.ModelsListConfig.Models[0])
