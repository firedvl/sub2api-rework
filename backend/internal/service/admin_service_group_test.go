...
func TestAdminGroupDisplayAndEnforcementRemainIndependent(t *testing.T) {
	repo := &groupRepoStubForAdmin{createID: 1}
	svc := &adminServiceImpl{groupRepo: repo}
	display := GroupModelsListConfig{Enabled: true, Models: []string{"display-model"}}
	policy := GroupModelAllowlist{Enabled: true, Models: []string{"allowed-model"}}
	group, err := svc.CreateGroup(context.Background(), &CreateGroupInput{
		Name: "independent", Platform: PlatformOpenAI, RateMultiplier: 1,
		ModelsListConfig: display, ModelAllowlist: policy,
	})
	require.NoError(t, err)
	require.Equal(t, display, group.ModelsListConfig)
	require.Equal(t, policy, group.ModelAllowlist)
	repo.getByID = group
	display.Models = []string{"new-display-model"}
	updated, err := svc.UpdateGroup(context.Background(), group.ID, &UpdateGroupInput{ModelsListConfig: &display})
	require.NoError(t, err)
	require.Equal(t, display, updated.ModelsListConfig)
	require.Equal(t, policy, updated.ModelAllowlist)
	require.True(t, updated.ModelAllowlist.Allows("allowed-model"))
	require.False(t, updated.ModelAllowlist.Allows("new-display-model"))
}
...
type groupRepoStubForAdmin struct {
	...
	createID      int64
	deleteIfEmpty func(context.Context, int64) ([]int64, error)
...
func (s *groupRepoStubForAdmin) DeleteCascadeIfEmpty(ctx context.Context, id int64) ([]int64, error) {
	if s.deleteIfEmpty != nil {
		return s.deleteIfEmpty(ctx, id)
	}
...
func TestAdminServiceSimpleModeValidatesRequestedGroupIDsDirectly(t *testing.T) {
	...
	require.NoError(t, svc.ValidateAccountGroupBindings(context.Background(), []int64{1, 1002}))
...
func TestAdminServiceSimpleModeAllowsDirectCompositeGroupAccess(t *testing.T) {
	...
	deleteCalls := 0
	repo.deleteIfEmpty = func(_ context.Context, id int64) ([]int64, error) {
		require.Equal(t, int64(9), id)
		deleteCalls++
		return nil, ErrGroupNotEmpty
	}
	...
	require.NoError(t, err)
	require.ErrorIs(t, svc.DeleteGroupIfEmpty(context.Background(), 9), ErrGroupNotEmpty)
	require.Equal(t, 1, deleteCalls)
}

func TestAdminServiceSimpleModeAllowsAccountListCompositeFilter(t *testing.T) {
	repo := &groupRepoStubForAdmin{getByID: &Group{ID: 9, Platform: PlatformComposite}}
	accounts := &accountRepoStubForAdminList{}
	svc := &adminServiceImpl{cfg: &config.Config{RunMode: config.RunModeSimple}, groupRepo: repo, accountRepo: accounts}
	_, _, err := svc.ListAccounts(context.Background(), 1, 20, "", "", "", "", 9, "", "", "")
	require.NoError(t, err)
	require.Equal(t, 1, accounts.listWithFiltersCalls)
}
...
func TestAdminServiceSimpleModeAllowsCompositeCreateAndExistingGroupUpdate(t *testing.T) {
	repo := &groupRepoStubForAdmin{getByID: &Group{ID: 1, Platform: PlatformComposite}}
	svc := &adminServiceImpl{cfg: &config.Config{RunMode: config.RunModeSimple}, groupRepo: repo}
	created, err := svc.CreateGroup(context.Background(), &CreateGroupInput{Platform: PlatformComposite, RateMultiplier: 1})
	require.NoError(t, err)
	require.Equal(t, PlatformComposite, created.Platform)

	updated, err := svc.UpdateGroup(context.Background(), 1, &UpdateGroupInput{Name: "updated"})
	require.NoError(t, err)
	require.Equal(t, PlatformComposite, updated.Platform)
