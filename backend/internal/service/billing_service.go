...
type ModelPricing struct {
	...
	ReasoningEffortMultipliers         map[string]float64 // 最终转发的推理等级对应的计费倍率
...
func reasoningEffortBillingMultiplier(model, effort string, multipliers map[string]float64) float64 {
	...
	multiplier, configured := multipliers[effort]
	if !configured && effort == "max" && isClaudeFable51Model(model) {
		return 3
	}
...
func (s *BillingService) initFallbackPricing() {
	...
	s.fallbackPrices["claude-opus-5-5"] = pricingWithPriorityMultiplier(&ModelPricing{
		...
		CacheCreationPricePerToken: 5e-6, CacheCreation5mPrice: 5e-6,
...
func (s *BillingService) CalculateCostUnified(input CostInput) (*CostBreakdown, error) {
	if input.Resolver == nil {
		...
		applyCostBreakdownMultiplier(breakdown, reasoningEffortBillingMultiplier(input.Model, input.ReasoningEffort, pricing.ReasoningEffortMultipliers))
	...
	case BillingModePerRequest, BillingModeImage, BillingModeVideo:
		...
		if err == nil && resolved.channelPricing != nil {
			applyCostBreakdownMultiplier(breakdown, reasoningEffortBillingMultiplier(input.Model, input.ReasoningEffort, resolved.channelPricing.ReasoningEffortMultipliers))
		}
