...
func TestOpenAIEncryptedLineageIsolatesAPIKeysAcrossIngressPaths(t *testing.T) {
	service := &OpenAIGatewayService{}
	contextForKey := func(id int64) *gin.Context {
		c, _ := gin.CreateTestContext(httptest.NewRecorder())
		c.Request = httptest.NewRequest("POST", "/v1/responses", nil)
		c.Request.Header.Set("session_id", "same-client-session")
		groupID := int64(7)
		c.Set("api_key", &APIKey{ID: id, GroupID: &groupID, Group: &Group{ID: groupID}})
		return c
	}
	first, second := contextForKey(11), contextForKey(12)
	payload := []byte(`{"input":[{"type":"reasoning","encrypted_content":"shared-cipher"}]}`)
	session := service.GenerateSessionHash(first, payload)
	require.Equal(t, session, service.GenerateSessionHash(second, payload))
	digests := collectOpenAIEncryptedContentDigestsRaw(payload)
	service.markOpenAIWSInvalidEncryptedContentLineage(first, 7, session, digests)
	require.NotEmpty(t, service.sessionInvalidEncryptedContentDigests(first, 7, session))
	require.Empty(t, service.sessionInvalidEncryptedContentDigests(second, 7, session))

	// HTTP bridge records the ingress hash through the same isolated owner.
	second.Set(openAIWSIngressSessionHashContextKey, session)
	bridgePayload := []byte(`{"input":[{"type":"compaction","encrypted_content":"bridge-cipher"}]}`)
	service.markOpenAIWSInvalidEncryptedContentLineageFromPayload(second, bridgePayload, "test_lineage", 1, 1)
	firstDigests := service.sessionInvalidEncryptedContentDigests(first, 7, session)
	secondDigests := service.sessionInvalidEncryptedContentDigests(second, 7, session)
	require.NotContains(t, firstDigests, openAIEncryptedContentDigest("bridge-cipher"))
	require.Contains(t, secondDigests, openAIEncryptedContentDigest("bridge-cipher"))
	require.NotContains(t, secondDigests, openAIEncryptedContentDigest("shared-cipher"))
}
