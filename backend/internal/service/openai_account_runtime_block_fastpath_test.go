...
type oauth429RateLimitRepo struct {
	...
	quotaRateLimitedCalls int
	lastQuotaLimitedUntil time.Time
	lastQuotaBlock        QuotaRateLimitBlock
}
...
func (r *oauth429RateLimitRepo) SetQuotaRateLimited(_ context.Context, _ int64, until time.Time, block QuotaRateLimitBlock) (bool, error) {
	r.quotaRateLimitedCalls++
	r.lastQuotaLimitedUntil = until
	r.lastQuotaBlock = block
	return true, nil
}

func (r *oauth429RateLimitRepo) ClearQuotaRateLimitIfObserved(context.Context, int64, QuotaRateLimitBlock) (bool, error) {
	return true, nil
}
...
func TestOpenAI429FastPath_BlocksOAuthImmediatelyWhenSevenDayQuotaIsExhausted(t *testing.T) {
	...
	require.False(t, shouldDisable)
	require.False(t, svc.isOpenAIAccountRuntimeBlocked(account), "the quota-owned runtime bridge clears after persistence")
	require.Zero(t, repo.setRateLimitedCalls)
	require.Equal(t, 1, repo.quotaRateLimitedCalls)
	require.Equal(t, "codex_headers", repo.lastQuotaBlock.Source)
	require.Equal(t, []string{"7d"}, repo.lastQuotaBlock.LimitingWindows)
...
func TestOpenAI429FastPath_RetryAfterExhaustionStaysGeneric(t *testing.T) {
	repo := &oauth429RateLimitRepo{}
	rateLimits := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
	svc := &OpenAIGatewayService{rateLimitService: rateLimits}
	rateLimits.SetAccountRuntimeBlocker(svc)
