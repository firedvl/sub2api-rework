...
// UpstreamModelInventorySnapshot is the durable account-scoped model identity
// inventory. It is independent from optional metadata completeness and from
// transient scheduler/quota state.
...
func (s *AccountTestService) SyncUpstreamModelCatalog(ctx context.Context, account *Account) (*UpstreamModelCatalog, error) {
	...
	inventory := UpstreamModelInventorySnapshot{Source: inventorySource, SyncedAt: time.Now().UTC().Format(time.RFC3339), Models: models}
	if len(body) > 0 {
	...
	if len(completeMetadata) > 0 && account != nil && account.ID > 0 && s.accountRepo != nil {
		...
		if err := s.accountRepo.UpdateExtra(ctx, account.ID, map[string]any{
			UpstreamModelInventoryExtraKey: inventory,
			UpstreamModelMetadataExtraKey:  snapshot,
		}); err != nil {
	...
	if !persistedCapabilities && account != nil && account.ID > 0 && s.accountRepo != nil {
		if err := s.accountRepo.UpdateExtra(ctx, account.ID, map[string]any{UpstreamModelInventoryExtraKey: inventory}); err != nil {
			return nil, newUpstreamModelSyncInternalError("Failed to save upstream model inventory", err)
		}
		account.SetUpstreamModelInventorySnapshot(inventory)
	}
