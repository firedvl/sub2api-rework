...
func (s *OpenAIGatewayService) markOpenAIWSInvalidEncryptedContentLineage(c *gin.Context, groupID int64, sessionHash string, digests []string) {
	...
	stateStore.MarkSessionInvalidEncryptedContent(groupID, isolateOpenAISessionID(getAPIKeyIDFromContext(c), sessionHash), digests, s.openAIWSSessionStickyTTL())
}

// sessionInvalidEncryptedContentDigests 返回会话已知失效密文摘要；全局无记录
// 时（常态）零成本返回 nil。
func (s *OpenAIGatewayService) sessionInvalidEncryptedContentDigests(c *gin.Context, groupID int64, sessionHash string) map[string]struct{} {
	...
	return stateStore.GetSessionInvalidEncryptedContentDigests(groupID, isolateOpenAISessionID(getAPIKeyIDFromContext(c), sessionHash))
}
