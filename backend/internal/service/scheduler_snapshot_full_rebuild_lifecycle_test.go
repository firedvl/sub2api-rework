...
func TestSchedulerFullRebuildActiveTombstoneDoesNotBlockFollowingGroupEvent(t *testing.T) {
	...
	require.Len(t, cache.tokens(), 2*len(schedulerBucketsForGroup(groupID)), "full rebuild and the following group event must each run fresh authority")
	_, reopenHeld := cache.lifecycleMutationLeaseStates()
	require.Len(t, reopenHeld, 2*len(schedulerBucketsForGroup(groupID)))
...
func TestSchedulerFullRebuildPartialLifecycleFailureReturnsBeforeDBAndRetries(t *testing.T) {
	...
	require.Equal(t, 3*len(schedulerCanonicalBuckets(0))+3, len(cache.retiredBuckets()))
...
func TestSchedulerFullRebuildSimpleModePreservesRegistryWithoutLifecycleAuthority(t *testing.T) {
	...
	require.Equal(t, expectedSchedulerAccountQueriesPerGroup()+3, accounts.callCount())
	require.Equal(t, expectedSchedulerAccountQueriesPerGroup()+3, accounts.groupCallCount(0))
	require.Empty(t, cache.retiredBuckets())
	require.Empty(t, cache.tokens())
