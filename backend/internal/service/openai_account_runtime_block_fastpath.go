...
func isRecoverableOpenAIQuota429(disposition openAIOAuth429Disposition, headers http.Header, responseBody []byte) bool {
	if strings.TrimSpace(headers.Get("Retry-After")) != "" {
		return false
	}
	return disposition == openAIOAuth429Quota5h || disposition == openAIOAuth429Quota7d ||
		(disposition == openAIOAuth429QuotaReset && isOpenAIUsageLimitReached(responseBody))
}
...
func (s *OpenAIGatewayService) markOpenAIOAuth429RateLimited(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	...
	if isRecoverableOpenAIQuota429(disposition, headers, responseBody) {
		s.BlockQuotaAccountScheduling(account, cooldownUntil)
	} else {
