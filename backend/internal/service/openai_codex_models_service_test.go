...
func TestOpenAIDurableCodexModelsManifestSnapshotsAreBounded(t *testing.T) {
	...
	oversized := []byte(`{"models":[]}` + strings.Repeat(" ", openAIModelsCacheBodyLimit))
...
func TestNewConfiguredCodexModelDescriptorUsesProviderMetadataAndSafeFallback(t *testing.T) {
	...
	require.Equal(t, "none", *gpt6Astra.DefaultReasoningLevel)
	require.Equal(t, []string{"none"}, effortsFromConfiguredCodexLevels(gpt6Astra.SupportedReasoningLevels))
	require.Nil(t, gpt6Astra.MultiAgentReasoningEffort)
	require.Nil(t, gpt6Astra.MultiAgentVersion)
	require.False(t, configuredCodexSupportsPriorityServiceTier("gpt-6-astra"))
	require.Empty(t, gpt6Astra.ServiceTiers)
	require.False(t, isOpenAICodexImageInputModel("gpt-6-astra"))
	require.False(t, isOpenAICodexReasoningGPTModel("openai/gpt-6-astra"))
	require.True(t, isOpenAIGPT6AstraModel("gpt-6-astra-2026-09-01"))
	require.False(t, isOpenAIGPT6AstraModel("gpt-6-other"))
	require.Equal(t, int64(configuredCodexFallbackContext), gpt6Astra.ContextWindow)
	require.Equal(t, int64(configuredCodexFallbackContext), gpt6Astra.MaxContextWindow)
	...
	require.Equal(t, []string{"none"}, effortsFromConfiguredCodexLevels(gpt6.SupportedReasoningLevels))
	require.Nil(t, gpt6.MultiAgentReasoningEffort)
	require.Equal(t, int64(configuredCodexFallbackContext), gpt6.ContextWindow)
...
// Scenario: local manifests do not invent upstream-only speed tiers.
...
func TestBuildGroupConfiguredCodexModelsManifestUsesAdministratorConfiguration(t *testing.T) {
	...
	require.NotContains(t, codexManifestModelSlugs(t, manifest.Body), "gpt-5.6-sol")
...
func TestCompleteAPIKeyCodexModelsManifestForClientMarksOnlyOfficialVisionGPTImageInput(t *testing.T) {
	...
	for _, slug := range []string{"gpt-6-astra", "gpt-3.5-turbo", "gpt-4"} {
...
func TestFetchCodexModelsManifestAPIKeySharedRefreshSurvivesCallerCancellation(t *testing.T) {
	...
	if remaining < 14*time.Second || remaining > openAIModelsRequestTimeout {
		t.Errorf("detached refresh deadline: got %s, want approximately %s", remaining, openAIModelsRequestTimeout)
	}
	cancelFirst()
	select {
	case err := <-firstErr:
