...
func TestAstraCodexToolCapabilitiesUseAccountScopeAndSharedDeclarations(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{"official host has no guessed capability", []Account{official}, false},
	...
	official.SetUpstreamModelMetadataSnapshot(UpstreamModelMetadataSnapshot{Models: map[string]UpstreamModelMetadata{
	...
	require.Nil(t, model["apply_patch_tool_type"], "shared null declaration must be retained")
...
// the operator has declared who serves the alias, so shared, account-synced
// reasoning metadata may survive. Image input remains disabled without the
// separate operator qualification.
func TestCodexAliasFailoverMappingHonorsModelRouting(t *testing.T) {
	reasoning := true
	newAccount := func(id int64, target string, priority int, withSnapshot bool) Account {
		account := Account{
			ID:       id,
			Platform: PlatformOpenAI,
			Type:     AccountTypeAPIKey,
	...
	// The alias resolves to two provider-specific targets. Neither target relies
	// on static Astra metadata, and both snapshots explicitly omit image input.
	primary := newAccount(1601, "provider-astra-primary", 0, true)
	failover := newAccount(1602, "provider-astra-failover", 10, true)
	accounts := []Account{primary, failover}
	unknownFailover := newAccount(1602, "provider-astra-failover", 10, false)

	// supported_reasoning_levels entries are {effort, description} objects; the
	// slider only cares about the effort names.
	manifestFieldsOf := func(t *testing.T, group *Group, candidates []Account) ([]string, any, any) {
		t.Helper()
		body, err := buildCodexModelsManifestForAccounts(
			PlatformOpenAI, []string{"gpt-6-astra"}, candidates, group, nil, true,
		)
		require.NoError(t, err)
		models := decodeCodexManifestModels(t, body)
		require.Len(t, models, 1)
	...
	t.Run("no routing rule still fails closed", func(t *testing.T) {
		levels, def, modalities := manifestFieldsOf(t, nil, []Account{primary, unknownFailover})
		require.Empty(t, levels, "ambiguous alias target must not advertise reasoning levels")
		require.Nil(t, def)
		require.Equal(t, []any{"text"}, modalities,
			"unqualified accounts must not advertise image input")
	})
	...
	t.Run("routing rule resolves the alias", func(t *testing.T) {
		group := &Group{
			...
			ModelRouting:        map[string][]int64{"gpt-6-astra": {1601, 1602}},
		}
		levels, def, modalities := manifestFieldsOf(t, group, accounts)
		require.Equal(t,
			[]string{"low", "medium", "high"},
			levels,
			"routed alias must keep the shared synced reasoning slider",
		)
	...
	t.Run("routing disabled falls back to failing closed", func(t *testing.T) {
		...
		levels, _, _ := manifestFieldsOf(t, group, []Account{primary, unknownFailover})
	...
	t.Run("routing rule for another alias does not leak", func(t *testing.T) {
		group := &Group{
			...
			Platform:            PlatformOpenAI,
			ModelRoutingEnabled: true,
			ModelRouting:        map[string][]int64{"gpt-5.6-sol": {1602}},
		}
		levels, _, _ := manifestFieldsOf(t, group, []Account{primary, unknownFailover})
