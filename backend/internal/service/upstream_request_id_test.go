...
func TestUpstreamRequestIDRejectsCredentialHeaders(t *testing.T) {
	for _, name := range []string{"Authorization", "PROXY-AUTHORIZATION", "Cookie", "Set-Cookie", "X-Api-Key", "Api-Key"} {
		...
		require.Error(t, ValidateUpstreamRequestIDHeaderExtra(extra))
		headers := http.Header{}
		headers.Set(name, "synthetic-private-value")
		require.Empty(t, UpstreamRequestIDFromHeaders(&Account{Extra: extra}, headers))
		require.Nil(t, usageUpstreamRequestIDPtr(&Account{Extra: extra}, headers, false))
	}
}
