...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if input.Extra != nil {
		...
		for _, key := range []string{
			...
			OpenAIAutoWarmupStateExtraKey,
			OpenAIAutoWarmupEvaluationExtraKey,
		} {
...
func (s *adminServiceImpl) ValidateAccountGroupBindings(ctx context.Context, groupIDs []int64) error {
	...
	for _, groupID := range groupIDs {
		...
		if !IsGroupBindableInSimpleMode(group) {
			return infraerrors.BadRequest("SIMPLE_MODE_GROUP_NOT_BINDABLE", "group cannot be bound in simple mode")
		}
	}
	return nil
}
