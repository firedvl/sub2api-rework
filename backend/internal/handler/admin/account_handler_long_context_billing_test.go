...
func TestApplyOAuthCredentialsPreservesNonAuthSettings(t *testing.T) {
	gin.SetMode(gin.TestMode)
	stub := newStubAdminService()
	stub.getAccountResult = &service.Account{
		ID: 1, Platform: service.PlatformOpenAI, Type: service.AccountTypeOAuth,
		Credentials: map[string]any{
			"access_token": "old", "model_mapping": map[string]any{"gpt-5": "gpt-5"},
			"account_id": "account-1", "password": "discard", "cookie": "discard",
		},
	}
	handler := NewAccountHandler(stub, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
	router := gin.New()
	router.POST("/accounts/:id/apply-oauth-credentials", handler.ApplyOAuthCredentials)
	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodPost, "/accounts/1/apply-oauth-credentials", bytes.NewBufferString(`{"type":"oauth","credentials":{"access_token":"new"}}`))
	request.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(recorder, request)

	require.Equal(t, http.StatusOK, recorder.Code)
	require.Equal(t, map[string]any{
		"access_token": "new", "model_mapping": map[string]any{"gpt-5": "gpt-5"}, "account_id": "account-1",
