...
func newSimpleModeGroupRouter(svc *stubAdminService) *gin.Engine {
	...
	r.GET("/groups/:id/model-allowlist-candidates", h.GetModelsListCandidates)
...
func TestGroupHandlerSimpleModePreservesCompositePlatform(t *testing.T) {
	svc := newStubAdminService()
	r := newSimpleModeGroupRouter(svc)
...
func TestGroupHandlerSimpleModeListsCompositeGroups(t *testing.T) {
	svc := newStubAdminService()
