...
const (
	...
	simpleModeGroupCompositeRoute simpleModeGroupOperation = "composite_route"
)
...
type CompositeRouteRequest struct {
	...
	Endpoint       string `json:"endpoint" binding:"omitempty,oneof=any messages count_tokens responses alpha_search chat_completions embeddings images gemini"`
...
func (h *GroupHandler) ListCompositeRoutes(c *gin.Context) {
	if h.rejectUnsupportedSimpleModeOperation(c, simpleModeGroupCompositeRoute) {
		return
	}
...
// GetModelsListCandidates serves the shared provider-backed candidate inventory
// for the independent display and enforcement editors.
