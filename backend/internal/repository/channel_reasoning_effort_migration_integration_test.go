...
func TestMigration247ReasoningEffortMultipliers(t *testing.T) {
	tx := testTx(t)
...
VALUES ($1, '["claude-fable-5-1"]') RETURNING id`, channelID).Scan(&unsetID))
	...
	migrationSQL, err := dbmigrations.FS.ReadFile("247_channel_reasoning_effort_multipliers.sql")
	require.NoError(t, err)
...
func TestMigration247GroupReasoningEffortMultipliers(t *testing.T) {
	tx := testTx(t)
	ctx := context.Background()
	var groupID int64
	require.NoError(t, tx.QueryRowContext(ctx, `
