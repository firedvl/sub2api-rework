...
func TestMigrationsRunner_UpgradeFrom244PreservesModerationAndAffiliateData(t *testing.T) {
	...
	require.Equal(t, 1, applied)
	require.NoError(t, upgradeDB.QueryRowContext(ctx, `SELECT count(*) FROM schema_migrations WHERE filename='247_channel_reasoning_effort_multipliers.sql'`).Scan(&applied))
	require.Equal(t, 1, applied)
