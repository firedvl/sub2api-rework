  ...
  <AppLayout>
      ...
      >
        ...
        >
          ...
          <template #cell-select="{ row }">
            <input type="checkbox" :checked="isSelected(row.id)" @change="toggleSel(row.id)" class="operator-checkbox rounded border-gray-300 focus:ring-gray-500" />
          </template>
          ...
          <template #cell-groups="{ row }">
            <AccountGroupsCell class="operator-account-routing-groups" :groups="accountGroupsForRow(row)" :max-display="4" />
          </template>
          ...
          <template #cell-scheduler_score="{ row }">
            <div v-if="getSchedulerScoreRows(row).length" class="flex min-w-[7rem] flex-col gap-0.5 font-mono text-[11px] leading-4">
              ...
              >
                ...
                <span class="text-gray-300 dark:text-gray-600">/</span>
                <span class="text-gray-900 dark:text-gray-100">{{ formatStickySchedulerScore(score) }}</span>
              </div>
            </div>
            <span v-else class="text-sm text-gray-400 dark:text-dark-500">-</span>
          </template>
    ...
    <ConfirmDialog :show="showExportDataDialog" :title="t('admin.accounts.dataExport')" :message="t('admin.accounts.dataExportConfirmMessage')" :confirm-text="t('admin.accounts.dataExportConfirm')" :cancel-text="t('common.cancel')" @confirm="handleExportData" @cancel="showExportDataDialog = false">
      <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
        <input type="checkbox" class="operator-checkbox h-4 w-4 rounded border-gray-300 focus:ring-gray-500" v-model="includeProxyOnExport" />
...
const closeImportData = () => {
  ...
  nextTick(() => accountToolsTriggerRef.value?.focus())
}
const handleDataImported = () => { closeImportData(); reload() }
