        ...
        <div class="border-t pt-4">
          <div class="mb-3 flex items-center justify-between gap-3">
            ...
            <Toggle
              ...
              :aria-label="t('admin.groups.modelAllowlist.title')"
            />
          </div>
          <div v-if="createModelAllowlistState.enabled" class="overflow-hidden rounded-lg border border-gray-200 bg-gray-50/50 dark:border-dark-600 dark:bg-dark-800/40">
            <div v-if="!createModelAllowlistLoading && createModelAllowlistState.items.length > 0" class="flex items-center justify-between gap-2 border-b border-gray-200 bg-gray-50 px-3 py-2 text-xs dark:border-dark-600 dark:bg-dark-800">
              <span class="text-gray-500 dark:text-gray-400">{{ t("admin.groups.modelAllowlist.selectedSummary", { selected: createModelAllowlistSelectedCount, total: createModelAllowlistState.items.length }) }}</span>
              <div class="flex items-center gap-1.5">
                <button type="button" class="rounded px-2 py-1 font-medium text-primary-600 hover:bg-primary-50 dark:text-primary-400 dark:hover:bg-primary-900/20" @click="selectAllModelAllowlistItems(createModelAllowlistState)">{{ t("admin.groups.modelAllowlist.selectAll") }}</button>
                <button type="button" class="rounded px-2 py-1 font-medium text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700" @click="invertModelAllowlistSelection(createModelAllowlistState)">{{ t("admin.groups.modelAllowlist.invertSelection") }}</button>
              </div>
            </div>
            <div class="max-h-64 space-y-2 overflow-y-auto p-2">
              <p v-if="createModelAllowlistLoading" class="text-xs text-gray-500 dark:text-gray-400">{{ t("admin.groups.modelAllowlist.loading") }}</p>
              <p v-else-if="createModelAllowlistState.items.length === 0" class="text-xs text-gray-500 dark:text-gray-400">{{ t("admin.groups.modelAllowlist.empty") }}</p>
              <div v-for="(item, index) in createModelAllowlistState.items" :key="item.id" class="flex items-center gap-2 rounded border border-gray-200 bg-white px-3 py-2 dark:border-dark-600 dark:bg-dark-800">
                <input v-model="item.selected" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500" />
                <span class="min-w-0 flex-1 break-all text-sm text-gray-700 dark:text-gray-300">{{ item.id }} <span v-if="item.id.endsWith('*')" class="ml-1 rounded bg-primary-50 px-1.5 py-0.5 text-[10px] font-medium text-primary-600 dark:bg-primary-900/30 dark:text-primary-400">{{ t("admin.groups.modelAllowlist.wildcardTag") }}</span></span>
                <button type="button" :disabled="index === 0" class="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-700 disabled:opacity-40 dark:hover:bg-dark-600 dark:hover:text-gray-200" @click="moveCreateModelAllowlistItem(index, index - 1)"><Icon name="arrowUp" size="sm" /></button>
                <button type="button" :disabled="index === createModelAllowlistState.items.length - 1" class="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-700 disabled:opacity-40 dark:hover:bg-dark-600 dark:hover:text-gray-200" @click="moveCreateModelAllowlistItem(index, index + 1)"><Icon name="arrowDown" size="sm" /></button>
              </div>
            </div>
            <div class="border-t border-gray-200 px-3 py-2 dark:border-dark-600">
              <div class="flex items-center gap-2"><input v-model="createAllowlistCustomEntry" type="text" :placeholder="t('admin.groups.modelAllowlist.customPlaceholder')" class="min-w-0 flex-1 rounded border border-gray-300 bg-white px-2 py-1.5 text-sm text-gray-700 focus:border-primary-500 focus:outline-none dark:border-dark-500 dark:bg-dark-700 dark:text-gray-200" @keydown.enter.prevent="submitCreateAllowlistCustomEntry" /><button type="button" class="rounded bg-primary-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-primary-700" @click="submitCreateAllowlistCustomEntry">{{ t("admin.groups.modelAllowlist.addCustom") }}</button></div>
              <p v-if="createAllowlistCustomErrorKey" class="mt-1 text-xs text-red-500">{{ t(createAllowlistCustomErrorKey) }}</p>
            </div>
          </div>
    ...
    >
      ...
      >
        ...
        <div class="border-t pt-4">
          ...
          <div v-if="editModelAllowlistState.enabled" class="overflow-hidden rounded-lg border border-gray-200 bg-gray-50/50 dark:border-dark-600 dark:bg-dark-800/40">
            <div v-if="!editModelAllowlistLoading && editModelAllowlistState.items.length > 0" class="flex items-center justify-between gap-2 border-b border-gray-200 bg-gray-50 px-3 py-2 text-xs dark:border-dark-600 dark:bg-dark-800">
              <span class="text-gray-500 dark:text-gray-400">{{ t("admin.groups.modelAllowlist.selectedSummary", { selected: editModelAllowlistSelectedCount, total: editModelAllowlistState.items.length }) }}</span>
              <div class="flex items-center gap-1.5">
                <button type="button" class="rounded px-2 py-1 font-medium text-primary-600 hover:bg-primary-50 dark:text-primary-400 dark:hover:bg-primary-900/20" @click="selectAllModelAllowlistItems(editModelAllowlistState)">{{ t("admin.groups.modelAllowlist.selectAll") }}</button>
                <button type="button" class="rounded px-2 py-1 font-medium text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700" @click="invertModelAllowlistSelection(editModelAllowlistState)">{{ t("admin.groups.modelAllowlist.invertSelection") }}</button>
              </div>
            </div>
            <div class="max-h-64 space-y-2 overflow-y-auto p-2">
              <p v-if="editModelAllowlistLoading" class="text-xs text-gray-500 dark:text-gray-400">{{ t("admin.groups.modelAllowlist.loading") }}</p>
              <p v-else-if="editModelAllowlistState.items.length === 0" class="text-xs text-gray-500 dark:text-gray-400">{{ t("admin.groups.modelAllowlist.empty") }}</p>
              <div v-for="(item, index) in editModelAllowlistState.items" :key="item.id" class="flex items-center gap-2 rounded border border-gray-200 bg-white px-3 py-2 dark:border-dark-600 dark:bg-dark-800">
                <input v-model="item.selected" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500" />
                <span class="min-w-0 flex-1 break-all text-sm text-gray-700 dark:text-gray-300">{{ item.id }} <span v-if="item.id.endsWith('*')" class="ml-1 rounded bg-primary-50 px-1.5 py-0.5 text-[10px] font-medium text-primary-600 dark:bg-primary-900/30 dark:text-primary-400">{{ t("admin.groups.modelAllowlist.wildcardTag") }}</span></span>
                <button type="button" :disabled="index === 0" class="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-700 disabled:opacity-40 dark:hover:bg-dark-600 dark:hover:text-gray-200" @click="moveEditModelAllowlistItem(index, index - 1)"><Icon name="arrowUp" size="sm" /></button>
                <button type="button" :disabled="index === editModelAllowlistState.items.length - 1" class="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-700 disabled:opacity-40 dark:hover:bg-dark-600 dark:hover:text-gray-200" @click="moveEditModelAllowlistItem(index, index + 1)"><Icon name="arrowDown" size="sm" /></button>
              </div>
            </div>
            <div class="border-t border-gray-200 px-3 py-2 dark:border-dark-600">
              <div class="flex items-center gap-2"><input v-model="editAllowlistCustomEntry" type="text" :placeholder="t('admin.groups.modelAllowlist.customPlaceholder')" class="min-w-0 flex-1 rounded border border-gray-300 bg-white px-2 py-1.5 text-sm text-gray-700 focus:border-primary-500 focus:outline-none dark:border-dark-500 dark:bg-dark-700 dark:text-gray-200" @keydown.enter.prevent="submitEditAllowlistCustomEntry" /><button type="button" class="rounded bg-primary-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-primary-700" @click="submitEditAllowlistCustomEntry">{{ t("admin.groups.modelAllowlist.addCustom") }}</button></div>
              <p v-if="editAllowlistCustomErrorKey" class="mt-1 text-xs text-red-500">{{ t(editAllowlistCustomErrorKey) }}</p>
            </div>
          </div>
...
type CodexManifestAccountsFieldExpose = { validate: () => boolean; resetValidation: () => void };
...
const submitCreateAllowlistCustomEntry = () => {
  const error = addCustomModelAllowlistItem(createModelAllowlistState, createAllowlistCustomEntry.value);
  if (error === null) {
...
const submitEditAllowlistCustomEntry = () => {
  const error = addCustomModelAllowlistItem(editModelAllowlistState, editAllowlistCustomEntry.value);
  if (error === null) {
...
const handleEdit = async (group: AdminGroup) => {
  ...
  const savedCodexManifestConfig = group.codex_models_manifest_config ?? createCodexManifestDefaults();
  ...
  for (const id of editCodexManifestConfig.value.account_ids) {
    void adminAPI.accounts.getById(id).then((account) => {
      editCodexManifestAccountNames.value = { ...editCodexManifestAccountNames.value, [id]: account.name };
    }).catch(() => undefined);
  }
  // 加载模型路由规则（异步加载账号名称）
...
const handleUpdateGroup = async () => {
  ...
  if (editForm.platform === "openai" && !editCodexManifestRef.value?.validate()) return;
  ...
  try {
    ...
    const payload = {
      ...
      codex_models_manifest_config: editForm.platform === "openai"
        ? { ...editCodexManifestConfig.value, account_ids: [...editCodexManifestConfig.value.account_ids] }
