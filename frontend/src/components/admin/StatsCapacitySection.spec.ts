...
describe('StatsCapacitySection', () => {
  ...
  it('separates actual windows and labels normalized donut semantics without turning unknown into zero', () => {
    ...
    expect(shortTerm.get('[data-testid="short-provider-openai"]').text()).toContain('Effective 5h')
    expect(shortTerm.get('[data-testid="short-provider-openai"]').text()).toContain('45%')
  ...
  it('keeps a 50+ account pool compact and limits Antigravity to three searchable model rows', async () => {
    ...
    const antigravityCapacity = wrapper.get('[data-testid="provider-capacity-antigravity"]')
    expect(antigravityCapacity.text()).toContain('Antigravity')
    expect(antigravityCapacity.text()).not.toContain('Google')
    expect(antigravityCapacity.findAll('dl > div')).toHaveLength(4)
    expect(antigravityCapacity.get('[data-testid="stats-capacity-donut-limit"]').text()).toContain('Antigravity pool')
    expect(antigravityCapacity.get('[data-testid="stats-capacity-donut-quota"]').text()).toContain('model-01 · +19')
    expect(antigravityCapacity.get('[data-testid="stats-capacity-donut-quota"]').attributes('title')).toContain('hidden-model-search-target')
    expect(antigravityCapacity.text()).not.toContain('model-02')
    expect(antigravityCapacity.text()).not.toContain('hidden-model-search-target')
    const antigravityIcon = antigravityCapacity.get('[data-provider-brand="antigravity"]')
    expect(antigravityIcon.attributes('fill')).toBe('currentColor')
    expect(antigravityIcon.attributes('viewBox')).toBe('0 0 112 112')
    expect(antigravityIcon.findAll('path')).toHaveLength(1)
    expect(antigravityIcon.get('path').attributes('d')).toMatch(/^M89\.6992 93\.695/)
    expect(antigravityIcon.html()).not.toMatch(/#[0-9a-f]{6}/i)
    expect(wrapper.find('[data-provider-brand="google"]').exists()).toBe(false)
    expect(wrapper.get('.stats-inspector-summary > div span').text()).toContain('Antigravity')
