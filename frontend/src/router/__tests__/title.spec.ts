...
describe('resolveDocumentTitle', () => {
  ...
  it('站点名为空时，回退默认站点名', () => {
    expect(resolveDocumentTitle('Dashboard', '')).toBe('Dashboard · Gateway')
    expect(resolveDocumentTitle(undefined, '   ')).toBe('Gateway')
    expect(resolveDocumentTitle('Overview', 'Sub2API')).toBe('Overview · Gateway')
  })

  it('页面标题与站点名相同时不重复', () => {
    expect(resolveDocumentTitle('Gateway', 'Sub2API')).toBe('Gateway')
    expect(resolveDocumentTitle('my site', 'My Site')).toBe('My Site')
  })
